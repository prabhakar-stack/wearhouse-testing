  ...
  const fetchLogs = async () => {
    try {
      ...
      for (const tid of eligibleTrackingIds) {
        ...
        const logId = firstClaim.trackingId || firstClaim.orderId || firstClaim.lpn || tid; // Use Tracking ID as primary log key
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <h4 className="text-sm font-bold text-slate-900">Tracking ID: {task.claimId}</h4>
                            ...
                            onClick={async (e) => {
                              ...
                              try {
                                const res = await fetch('/api/bot/trigger', {
                                  method: 'POST',
                                  headers: { 'Content-Type': 'application/json' },
                                  body: JSON.stringify({ trackingId: task.claimId, orderId: task.claimId })
                                });
                                const data = await res.json();
                                if (res.ok) {
                                  alert(`Bot triggered for Tracking ID: ${task.claimId}`);
                                } else {
