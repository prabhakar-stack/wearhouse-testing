...
export default function SmartFiling() {
  ...
  const triggerTest = async () => {
    if (!testOrderId) return alert(t("Please provide an Order ID or LPN"));
    try {
      ...
      if (res.ok) {
        alert(t("Bot triggered successfully! View progress in the Live Monitor below."));
        setTestOrderId('');
        fetchLogs();
        fetchConfig();
      } else {
        alert(data.message || t("Failed to trigger bot"));
      }
    } catch (err) {
      alert(t("Failed to connect to the bot server."));
      console.error(err);
    }
  };
  ...
  const fetchLogs = async () => {
    try {
      ...
      for (const tid of eligibleTrackingIds) {
        ...
        const logId = firstClaim.orderId || firstClaim.lpn || tid; 
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl lg:text-2xl font-bold tracking-tight">{t("Smart Filing Hub")}</h2>
          </div>
          <p className="text-slate-500 text-xs lg:text-sm italic">{t("Automated Amazon filing bot powered by Playwright.")}</p>
        </div>

        <div className="flex items-center gap-3">
          <div className="flex flex-col items-end px-4 border-r border-slate-200">
            <span className="text-[10px] uppercase font-black text-slate-400">{t("Bot Health")}</span>
            {config.isBotRunning ? (
              <span className="text-sm font-bold text-blue-600 flex items-center gap-1">
                <Loader2 className="w-3.5 h-3.5 animate-spin" /> {t("BUSY")}
              </span>
            ) : config.coolingRemainingMs > 0 ? (
              <span className="text-sm font-bold text-amber-600 flex items-center gap-1">
                <History className="w-3.5 h-3.5" /> {t("COOLING")}
              </span>
            ) : (
              <span className="text-sm font-bold text-green-600 flex items-center gap-1">
                <ShieldCheck className="w-3.5 h-3.5" /> {t("ONLINE")}
              </span>
            )}
          </div>
          <button 
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-6">
            ...
            <div className="bg-white border-2 border-indigo-600 rounded-2xl overflow-hidden shadow-xl animate-in fade-in slide-in-from-top-4 duration-500">
              <div className="bg-indigo-600 px-4 py-2 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black text-white uppercase tracking-widest">{t("Live Bot Monitor")}</span>
                </div>
                <span className="text-[10px] font-bold text-indigo-100 italic">{t("Showing browser workspace...")}</span>
              </div>
              <div className="aspect-video bg-slate-100 flex items-center justify-center relative group">
                ...
                <div className="absolute bottom-4 right-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg border border-white/10 flex items-center gap-2">
                   ...
                   <span className="text-[9px] font-bold text-white uppercase tracking-tighter">{t("Encrypted Stream")}</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-2 mb-2 px-2">
            ...
            <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t("Active Automation Queue")}</span>
          </div>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm min-h-[200px]">
            {tasks.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-20 text-center">
                ...
                <p className="text-slate-400 text-sm font-medium">{t("No active or recent automated tasks found.")}</p>
                <p className="text-slate-300 text-xs mt-1">{t("Trigger a filing from the Triage Queue to see logs here.")}</p>
              </div>
            ) : (
                  ...
                  <div key={task.id} className="p-4 lg:p-6 hover:bg-slate-50/50 transition-colors">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-bold text-slate-400">/ {t(task.timestamp)}</span>
                          </div>
                          <h4 className="text-sm font-bold text-slate-900">{t("Order ID:")} {task.claimId}</h4>
                      ...
                      <div className="flex items-center gap-2">
                        {task.status === 'Queued' && (
                          <button 
                            ...
                            onClick={async (e) => {
                              ...
                              try {
                                ...
                                const data = await res.json();
                                if (res.ok) {
                                  alert(`${t("Bot triggered for Order:")} ${task.claimId}`);
                                } else {
                                  alert(data.message || t("Failed to start bot."));
                                }
                                fetchLogs();
                                fetchConfig();
                              } catch (e) {
                          ...
                          >
                            {config.isBotRunning ? t('BOT BUSY') : config.coolingRemainingMs > 0 ? t('COOLING') : t('RUN FILING')}
                          </button>
                        )}
                    ...
                    <div className="bg-[#0D1117] rounded-xl p-4 font-mono text-[10px] leading-relaxed text-slate-300 border border-slate-800">
                      <div className="flex items-center gap-2 mb-2 border-b border-slate-800 pb-2">
                        ...
                        <span className="text-indigo-400 uppercase font-bold tracking-widest">{t("Bot Execution Logs")}</span>
                      </div>
                      <div className="space-y-1 max-h-[150px] overflow-y-auto custom-scrollbar">
                        ...
                        {task.status === 'Running' && (
                          <div className="flex gap-2 animate-pulse text-indigo-400">
                            ...
                            <span>{t("_ Executing next instruction...")}</span>
                          </div>
                        )}
                      </div>
                    </div>
        ...
        <div className="space-y-6">
          <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-[#313079]">{t("Bot Configuration")}</h3>
              {config.configured ? (
                <span className="text-[10px] bg-green-50 text-green-600 px-2 py-0.5 rounded-full font-black border border-green-100">{t("READY")}</span>
              ) : (
                <span className="text-[10px] bg-red-50 text-red-600 px-2 py-0.5 rounded-full font-black border border-red-100">{t("SETUP REQUIRED")}</span>
              )}
            </div>
            <div className="space-y-4">
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">{t("Target Account")}</p>
                <p className="text-xs font-bold text-slate-900">{config.email || 'Not configured in .env'}</p>
              </div>
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                <div>
                  <p className="text-[10px] font-bold text-slate-400 uppercase">{t("TOTP Secret")}</p>
              ...
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                <div>
                  <p className="text-[10px] font-bold text-slate-400 uppercase">{t("Headless Mode")}</p>
                  <p className="text-xs font-bold text-slate-900">{config.headless ? t('Enabled') : t('Disabled (Headed)')}</p>
                </div>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
            <h3 className="text-sm font-bold text-[#313079] mb-4">{t("Manual Test Trigger")}</h3>
            <div className="space-y-3">
              <input 
                type="text" 
                placeholder={t("Amazon Order ID / Tracking ID / LPN")}
              ...
              >
                {config.isBotRunning ? (
                  <><Loader2 className="w-3.5 h-3.5 animate-spin" /> {t("BOT BUSY")}</>
                ) : config.coolingRemainingMs > 0 ? (
                  <><History className="w-3.5 h-3.5" /> {t("COOLING")} ({Math.ceil(config.coolingRemainingMs / 1000 / 60)}m)</>
                ) : (
                  <><Play className="w-3.5 h-3.5" /> {t("RUN TEST")}</>
                )}
              </button>
            </div>
          </div>
          ...
          <div className="bg-indigo-900 rounded-2xl p-6 text-white shadow-lg shadow-indigo-100">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-sm font-bold">{t("Security Notice")}</h3>
            </div>
            <p className="text-[11px] leading-relaxed text-indigo-100/70">
              {t("Bot actions are recorded for audit purposes. Ensure your TOTP secret is stored in encrypted environment variables before production.")}
            </p>
          </div>
        </div>
      </div>
