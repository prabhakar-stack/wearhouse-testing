...
export async function GET(req: Request) {
  try {
    ...
    for (const manifest of overdueManifests) {
      ...
      const hoursOverdue =
        (now.getTime() - new Date(manifest.expectedDate).getTime()) / (1000 * 60 * 60);
      ...
      const alert = await createAlertIfNew(alertType, manifest.id, manifest.trackingId);
    ...
    for (const manifest of dockManifests) {
      ...
      const daysSinceReceipt = Math.floor(
        (today.getTime() - new Date(manifest.receivedAt).getTime()) / (1000 * 60 * 60 * 24),
      );
      ...
      if (daysSinceReceipt >= 2) {
        ...
        if (currentHour >= 15) alertType = 'RECV_INSP_HANDSHAKE_3PM';        // L3 ≥15:00
        else if (currentHour >= 12) alertType = 'RECV_INSP_HANDSHAKE_12PM'; // L2 ≥12:00
    ...
    for (const manifest of claimsManifests) {
      ...
      let alertType: string | null = null;
      if (hoursInStaging >= 24) alertType = 'INSPECTION_QC_FAILED_24H';       // L4
      else if (hoursInStaging >= 12) alertType = 'INSPECTION_QC_FAILED_12H';  // L3
