...
export default function ClaimsSpecialistDashboard({
  ...
  const [activeTab, setActiveTab] = useState<"home" | "alerts" | "triage" | "smartfiling" | "profile">("home");
  ...
  return (
    <div className="h-screen w-screen bg-white text-[#313079] font-sans flex flex-col lg:flex-row overflow-hidden relative select-none">
      ...
      >
        ...
        <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
          ...
          <TabButton 
            ...
            label={preferredLanguage === 'hi' ? 'दावा ट्रियेज' : 'Claims Triage'} 
            activeTab={activeTab} 
            setActive={(tab: any) => { setActiveTab(tab); setIsMobileMenuOpen(false); }} 
            isMinimized={isSidebarMinimized} 
          />
          <TabButton 
            ...
            label={preferredLanguage === 'hi' ? 'स्मार्ट फाइलिंग मॉनिटर' : 'Smart Filing Monitor'} 
            activeTab={activeTab} 
            setActive={(tab: any) => { setActiveTab(tab); setIsMobileMenuOpen(false); }} 
            isMinimized={isSidebarMinimized} 
          />
        ...
        <div className={`p-4 border-t border-white/10 shrink-0 ${isSidebarMinimized ? 'flex flex-col items-center space-y-4' : 'space-y-3'}`}>
          {!isSidebarMinimized && (role === 'ADMIN' || role === 'SUPER_ACCESS') && (
            <div className="flex flex-col space-y-1.5 px-2 w-full animate-in fade-in duration-200">
              ...
              <div className="relative">
                <select
                  value={typeof window !== 'undefined' ? (localStorage.getItem('userRole') || 'CLAIMS_SPECIALIST') : 'CLAIMS_SPECIALIST'}
                  onChange={(e) => {
          ...
          >
            <div className="shrink-0 w-8 h-8 rounded-full bg-[#FF6700]/10 border border-[#FF6700]/30 flex items-center justify-center text-[#FF6700] text-xs font-black">
              {name ? name.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2) : "CS"}
            </div>
            {!isSidebarMinimized && (
      ...
      {/* Content Area */}
      <main className={`flex-1 flex flex-col min-w-0 ${activeTab === 'triage' || activeTab === 'smartfiling' || activeTab === 'alerts' ? 'overflow-hidden p-0 bg-slate-50' : 'overflow-y-auto p-6 bg-[#FF6700]/5'}`}>
        {activeTab === "home" && (
            ...
            <div className="bg-gradient-to-br from-slate-900 to-indigo-950 border border-slate-800 text-white rounded-2xl p-8 relative overflow-hidden shadow-xl">
              ...
              <div className="relative z-10 space-y-4">
                ...
                <h1 className="text-3xl md:text-4xl font-black tracking-wide uppercase leading-tight text-white">
                  {preferredLanguage === 'hi' ? 'क्लेम्स स्पेशलिस्ट वर्कस्पेस' : 'Claims Specialist Workspace'}
                </h1>
                <p className="text-slate-300 text-sm max-w-xl leading-relaxed uppercase tracking-wider font-bold">
                  {preferredLanguage === 'hi'
                    ? 'डैशबोर्ड में आपका स्वागत है। यहां से दावे ट्रियेज, स्मार्ट फाइलिंग मॉनिटर और सक्रिय परिचालन अलर्ट प्रबंधित करें।'
                    : 'Welcome to your dashboard. Access claims triage grids, smart filing monitors, and execute automated operational exception audits.'}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col justify-between space-y-4 hover:shadow-md hover:-translate-y-1 transition-all duration-300 group">
                <div className="space-y-2">
                  ...
                  <h3 className="text-sm font-black uppercase text-[#313079]">
                    {preferredLanguage === 'hi' ? 'दावा ट्रियेज' : 'Claims Triage'}
                  </h3>
                  <p className="text-xs text-slate-500 leading-normal font-medium">
                    {preferredLanguage === 'hi'
                      ? 'असंगत या दोषपूर्ण पैकेजों के लिए Amazon रिम्बर्समेंट दावों का निरीक्षण और निपटान करें।'
                      : 'Inspect and resolve Amazon reimbursement claims for discrepant or failed packages.'}
                  </p>
                </div>
                <button
                ...
                >
                  {preferredLanguage === 'hi' ? 'ट्रियेज पर जाएं' : 'Go to Triage'}
                </button>
              </div>
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col justify-between space-y-4 hover:shadow-md hover:-translate-y-1 transition-all duration-300 group">
                <div className="space-y-2">
                  ...
                  <h3 className="text-sm font-black uppercase text-[#313079]">
                    {preferredLanguage === 'hi' ? 'स्मार्ट फाइलिंग' : 'Smart Filing Monitor'}
                  </h3>
                  <p className="text-xs text-slate-500 leading-normal font-medium">
                    {preferredLanguage === 'hi'
                      ? 'वास्तविक समय में स्वचालित क्लेम फाइलिंग प्रगति और कूरियर स्टेटस की निगरानी करें।'
                      : 'Monitor automated claim filing progress and courier API integrations in real-time.'}
                  </p>
                </div>
                <button
                  ...
                  className="w-full py-2.5 bg-slate-100 hover:bg-slate-200 text-[#313079] text-xs font-black uppercase tracking-wider rounded-lg transition-colors"
                >
                  {preferredLanguage === 'hi' ? 'मॉनिटर खोलें' : 'Open Monitor'}
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="border border-slate-100 rounded-xl p-4 bg-slate-50/50">
                  <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider">
                    {preferredLanguage === 'hi' ? 'लंबित दावे' : 'Pending Claims'}
                  </p>
                ...
                <div className="border border-slate-100 rounded-xl p-4 bg-slate-50/50">
                  <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider">
                    {preferredLanguage === 'hi' ? 'विवाद' : 'Disputes'}
                  </p>
              ...
              <div className="bg-gradient-to-r from-blue-600 to-indigo-600 h-32 relative">
                <div className="absolute -bottom-10 left-8">
                  <div className="w-20 h-20 rounded-2xl bg-white p-1 shadow-lg">
                    <div className="w-full h-full rounded-xl bg-blue-100 flex items-center justify-center text-blue-600 text-2xl font-black">
                      {name ? name.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2) : "CS"}
                    </div>
                  </div>
                </div>
