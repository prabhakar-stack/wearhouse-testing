...
import ClaimsSpecialistDashboard from './ClaimsSpecialistDashboard';
import { AlertOctagon } from 'lucide-react';
import { prisma } from '@/lib/prisma';

export default async function ClaimsSpecialistPage() {
  const headersList = await headers();
  const userId = headersList.get('x-user-id') || '';
  const role = headersList.get('x-user-role') || '';
  const email = headersList.get('x-user-email') || '';
  
  if (role !== 'CLAIMS_SPECIALIST' && role !== 'ADMIN' && role !== 'SUPER_ACCESS') {
    return (
  ...
  return <ClaimsSpecialistDashboard userId={userId} role={role} name={resolvedName} email={email} />;
}
