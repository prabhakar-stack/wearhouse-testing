...
export default function RecovererDashboard({
  ...
  const [activeTab, setActiveTab] = useState<"home" | "recovery" | "alerts" | "profile">("home");
  ...
  const recoveryUrl = process.env.NEXT_PUBLIC_CLAIMS_PROCESS_URL || "http://localhost:5000";
  ...
  return (
    <div className="h-screen w-screen bg-white text-[#313079] font-sans flex flex-col lg:flex-row overflow-hidden relative select-none">
      ...
      >
        ...
        <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
          ...
          <TabButton 
            ...
            label={preferredLanguage === 'hi' ? 'रिकवरी हब' : 'Recovery Hub'} 
            activeTab={activeTab} 
            setActive={(tab: any) => { setActiveTab(tab); setIsMobileMenuOpen(false); }} 
            isMinimized={isSidebarMinimized} 
          />
        ...
        <div className={`p-4 border-t border-white/10 shrink-0 ${isSidebarMinimized ? 'flex flex-col items-center space-y-4' : 'space-y-3'}`}>
          ...
          >
            <div className="shrink-0 w-8 h-8 rounded-full bg-[#FF6700]/10 border border-[#FF6700]/30 flex items-center justify-center text-[#FF6700] text-xs font-black">
              {name ? name.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2) : "RC"}
            </div>
            {!isSidebarMinimized && (
      ...
      {/* Content Area */}
      <main className={`flex-1 flex flex-col min-w-0 ${activeTab === 'recovery' || activeTab === 'alerts' ? 'overflow-hidden p-0 bg-slate-50' : 'overflow-y-auto p-6 bg-[#FF6700]/5'}`}>
        {activeTab === "home" && (
          <div className="max-w-4xl mx-auto space-y-6 pt-10 px-6 pb-10 text-left w-full animate-in fade-in duration-300">
            ...
            <div className="bg-gradient-to-br from-slate-900 to-indigo-950 border border-slate-800 text-white rounded-2xl p-8 relative overflow-hidden shadow-xl">
              <div className="absolute right-0 bottom-0 opacity-10 translate-x-10 translate-y-10">
                <RefreshCw size={250} className="animate-spin-slow" />
              </div>
              <div className="relative z-10 space-y-4">
                <span className="px-3 py-1 bg-[#FF6700]/20 text-[#FF6700] rounded-full text-[10px] font-black uppercase tracking-widest border border-[#FF6700]/30 animate-pulse">
                  {preferredLanguage === 'hi' ? 'रिकवरी सेंटर' : 'Recovery Hub'}
                </span>
                <h1 className="text-3xl md:text-4xl font-black tracking-wide uppercase leading-tight text-white">
                  {preferredLanguage === 'hi' ? 'रिकवरर वर्कस्पेस' : 'Recoverer Workspace'}
                </h1>
                <p className="text-slate-300 text-sm max-w-xl leading-relaxed uppercase tracking-wider font-bold">
                  {preferredLanguage === 'hi'
                    ? 'डैशबोर्ड में आपका स्वागत है। यहाँ से उत्पाद रिकवरी, SKU सुधार, और संबंधित अलर्ट प्रबंधित करें।'
                    : 'Welcome to your dashboard. Process unit recoveries, inspect damaged SKU handovers, and resolve active compliance alerts.'}
                </p>
              </div>
            </div>
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col justify-between space-y-4 hover:shadow-md hover:-translate-y-1 transition-all duration-300 group">
                <div className="space-y-2">
                  ...
                  <h3 className="text-sm font-black uppercase text-[#313079]">
                    {preferredLanguage === 'hi' ? 'रिकवरी हब' : 'Recovery Hub'}
                  </h3>
                  <p className="text-xs text-slate-500 leading-normal font-medium">
                    {preferredLanguage === 'hi'
                      ? 'क्षतिग्रस्त, खोए हुए, या पुनर्प्राप्त करने योग्य SKU के क्लेम और स्थिति को प्रोसेस करें।'
                      : 'Process claims and track recovery pipelines for returned SKU items and warehouse damage.'}
                  </p>
                </div>
                <button
                ...
                >
                  {preferredLanguage === 'hi' ? 'रिकवरी हब पर जाएं' : 'Go to Recovery'}
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="border border-slate-100 rounded-xl p-4 bg-slate-50/50">
                  <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider">
                    {preferredLanguage === 'hi' ? 'रिकवरी में आइटम' : 'Items in Recovery'}
                  </p>
                  <p className="text-2xl font-black font-mono text-[#313079] mt-1">0</p>
                </div>
                <div className="border border-slate-100 rounded-xl p-4 bg-slate-50/50">
                  <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider">
                    {preferredLanguage === 'hi' ? 'इस सप्ताह रिकवर' : 'Recovered This Week'}
                  </p>
                  <p className="text-2xl font-black font-mono text-green-600 mt-1">0</p>
                </div>
                <div className="border border-slate-100 rounded-xl p-4 bg-slate-50/50">
                  <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider">
                    {preferredLanguage === 'hi' ? 'असफल रिकवरी' : 'Failed Recoveries'}
                  </p>
        ...
        {activeTab === "recovery" && (
          <iframe
            src={`${recoveryUrl}/recoveryhubtab?lang=${preferredLanguage}`}
              ...
              <div className="bg-gradient-to-r from-[#313079] to-indigo-950 h-32 relative">
                <div className="absolute -bottom-10 left-8">
                  <div className="w-20 h-20 rounded-2xl bg-white p-1 shadow-lg">
                    <div className="w-full h-full rounded-xl bg-purple-100 flex items-center justify-center text-purple-600 text-2xl font-black">
                      {name ? name.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2) : "RC"}
                    </div>
                  </div>
                </div>
