    ...
    <div className="min-h-screen bg-[#0A0A0A] text-[#E0E0E0] p-6 lg:p-12 font-sans selection:bg-[#C5A059] selection:text-white border-8 border-[#1A1A1A] flex flex-col h-screen overflow-hidden">
      ...
      <header className="flex justify-between items-center mb-8 shrink-0">
        <div className="flex items-center space-x-4">
          <div className="w-10 h-10 bg-[#FF4444] flex items-center justify-center shadow-[0_0_15px_rgba(255,68,68,0.3)]">
            <Shield className="text-[#0A0A0A]" size={24} strokeWidth={2.5} />
          </div>
          <div>
            <h1 className="text-2xl font-serif italic text-[#F5F2ED] tracking-wide">Supreme Command Center</h1>
        ...
        <div className="flex bg-[#111111] border border-[#333333] rounded-sm p-1 items-center">
          ...
          <Link href="/receiver" className="text-[#888888] hover:text-[#FF4444] text-[10px] uppercase font-bold tracking-widest px-2 transition-colors">Receiver</Link>
          <Link href="/inspector" className="text-[#888888] hover:text-[#FF4444] text-[10px] uppercase font-bold tracking-widest px-2 transition-colors">Inspector</Link>
          <div className="h-4 w-px bg-[#333333] mx-2"></div>
          <button 
            onClick={async () => {
              try {
  ...
  return (
    <button 
      ...
      className={`flex items-center space-x-2 px-6 py-2.5 text-xs font-medium tracking-wider uppercase transition-all duration-300 ${
        isActive ? 'bg-[#1A1A1A] text-[#FF4444] border border-[#FF4444]/30 shadow-[0_0_10px_rgba(255,68,68,0.1)]' : 'text-[#666666] hover:text-[#E0E0E0] hover:bg-[#151515] border border-transparent'
      }`}
    >
    ...
    <div className="flex flex-col h-full p-8 space-y-8 overflow-y-auto custom-scrollbar">
      <div className="flex justify-between items-end">
        <div>
           ...
           <p className="text-[#888888] text-xs tracking-wider mt-1">Manage personnel access and roles globally.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 border border-[#333333] bg-[#0A0A0A] p-6 h-fit">
          <h3 className="text-sm uppercase tracking-widest text-[#FF4444] mb-6">Authorize Personnel</h3>
          <form onSubmit={handleCreate} className="space-y-4">
            <div>
              <label className="block text-[10px] uppercase tracking-wider text-[#666666] mb-1">Email Address</label>
              <input type="email" required value={email} onChange={e => setEmail(e.target.value)} 
                className="w-full bg-[#111111] border border-[#333333] text-[#E0E0E0] px-4 py-2 text-sm focus:border-[#FF4444] focus:outline-none transition-colors" />
            </div>
            <div>
            ...
            <button type="submit" className="w-full mt-4 bg-[#1A1A1A] border border-[#333333] hover:border-[#FF4444] text-[#E0E0E0] px-4 py-3 text-xs uppercase tracking-widest transition-colors font-medium">
              Grant Access
            </button>
          </form>
        </div>
                ...
                {loading ? (
                  ...
                  <tr key={user.id} className="hover:bg-[#111111]/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className="bg-[#1A1A1A] border border-[#333333] px-2 py-1 text-[10px] tracking-wide uppercase text-[#FF4444]">
                        {user.role.replace('_', ' ')}
                      </span>
                    </td>
    ...
    <div className="flex flex-col h-full relative">
      <div className="p-8 border-b border-[#333333] shrink-0">
        ...
        <form onSubmit={handleSearch} className="flex max-w-xl">
          <div className="flex-1 flex bg-[#0A0A0A] border border-[#333333] focus-within:border-[#FF4444] transition-colors">
          ...
          <button type="submit" disabled={loading} className="px-8 bg-[#1A1A1A] border-y border-r border-[#333333] hover:bg-[#FF4444] hover:text-[#0A0A0A] transition-colors uppercase tracking-widest text-[11px] font-bold">
            Track
          </button>
        </form>
          ...
          <div className="max-w-2xl mx-auto py-4">
            <div className="mb-12 flex justify-between items-end border-b border-[#333333] pb-6">
              ...
              <div className="text-right">
                ...
                <p className="text-[#FF4444] text-xs tracking-wider uppercase font-medium">{manifest.status.replace(/_/g, ' ')}</p>
              </div>
            </div>
            ...
            <div className="relative border-l border-[#333333] ml-4 space-y-12 pb-12">
              {events.map((ev, i) => (
                <div key={i} className="relative pl-8 group">
                  <div className="absolute -left-2 top-1.5 w-4 h-4 bg-[#111111] border-2 border-[#FF4444] rounded-full shadow-[0_0_8px_rgba(255,68,68,0.5)]"></div>
                  ...
                  <div className="flex items-center space-x-3 mt-1.5">
                    ...
                    <button onClick={() => openEditor(ev.id, ev.type, ev.time)} className="p-1 px-3 bg-[#111111] border border-[#333333] text-[#FF4444] hover:bg-[#FF4444] hover:text-[#0A0A0A] transition-all text-[10px] uppercase tracking-widest font-bold flex items-center space-x-1" title="Amend Timestamp">
      ...
      {editModal && (
        <div className="absolute inset-0 bg-[#0A0A0A]/80 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="bg-[#111111] border border-[#333333] p-8 w-[400px] shadow-2xl">
            <div className="flex justify-between items-center mb-6 border-b border-[#1A1A1A] pb-4">
              <h3 className="text-sm uppercase tracking-widest text-[#FF4444]">Amend Timestamp</h3>
              <button onClick={() => setEditModal(null)} className="text-[#666666] hover:text-white"><X size={16} /></button>
            </div>
            <p className="text-[10px] uppercase text-[#888888] mb-6 leading-relaxed">
              WARNING: Manually mutating timestamps alters the immutable timeline. This action is logged.
            </p>
            <div className="space-y-4">
              <div>
                <label className="block text-[10px] uppercase tracking-wider text-[#666666] mb-1">Date</label>
                <input type="date" value={editDate} onChange={e => setEditDate(e.target.value)}
                  className="w-full bg-[#0A0A0A] border border-[#333333] text-[#E0E0E0] px-4 py-2 text-sm focus:border-[#FF4444] outline-none" />
              </div>
              <div>
                <label className="block text-[10px] uppercase tracking-wider text-[#666666] mb-1">Time (24H)</label>
                <input type="time" value={editTime} onChange={e => setEditTime(e.target.value)}
                  className="w-full bg-[#0A0A0A] border border-[#333333] text-[#E0E0E0] px-4 py-2 text-sm focus:border-[#FF4444] outline-none" />
              </div>
              <button onClick={handleUpdateTimestamp} className="w-full mt-4 bg-[#FF4444]/10 border border-[#FF4444]/50 hover:bg-[#FF4444]/20 text-[#FF9999] px-4 py-3 text-xs uppercase tracking-widest font-medium transition-colors flex justify-center items-center space-x-2">
            ...
            {loading ? (
              ...
              return (
                <tr key={c.id} className="hover:bg-[#111111]/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    {ev ? (
                      <a href={ev} target="_blank" rel="noreferrer" className="inline-flex items-center space-x-1 text-[#FF4444] hover:text-white text-xs border border-[#FF4444]/30 px-2 py-1 bg-[#1A1A1A]">
