    ...
    <div className="h-screen w-screen bg-white text-[#313079] font-sans flex flex-col lg:flex-row overflow-hidden relative">
      ...
      >
        ...
        <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
          <TabButton id="users"    icon={<Users size={14} />}       label={lang === 'hi' ? 'उपयोगकर्ता' : 'Users'}          activeTab={activeTab} setActive={(tab: any) => { setActiveTab(tab); setIsMobileMenuOpen(false); }} isMinimized={isSidebarMinimized} />
          <TabButton id="alerts"   icon={<Bell size={14} />}        label={lang === 'hi' ? 'अलर्ट' : 'Alerts'}         activeTab={activeTab} setActive={(tab: any) => { setActiveTab(tab); setIsMobileMenuOpen(false); }} badge={alertCount > 0 ? alertCount : undefined} isMinimized={isSidebarMinimized} />
          <TabButton id="claims"   icon={<FileWarning size={14} />} label={lang === 'hi' ? 'दावे' : 'Claims'}         activeTab={activeTab} setActive={(tab: any) => { setActiveTab(tab); setIsMobileMenuOpen(false); }} isMinimized={isSidebarMinimized} />
          {canAccessTriage && (
            <TabButton id="triage" icon={<FileWarning size={14} />} label={lang === 'hi' ? 'दावा ट्रियेज' : 'Claims Triage'} activeTab={activeTab} setActive={(tab: any) => { setActiveTab(tab); setIsMobileMenuOpen(false); }} isMinimized={isSidebarMinimized} />
          )}
          {canAccessSmartFiling && (
            <TabButton id="smart-filing" icon={<Activity size={14} />} label={lang === 'hi' ? 'स्मार्ट फाइलिंग मॉनिटर' : 'Smart Filing Monitor'} activeTab={activeTab} setActive={(tab: any) => { setActiveTab(tab); setIsMobileMenuOpen(false); }} isMinimized={isSidebarMinimized} />
          )}
          {canAccessRecovery && (
            <TabButton id="recovery" icon={<PackageSearch size={14} />} label={lang === 'hi' ? 'रिकवरी हब' : 'Recovery Hub'} activeTab={activeTab} setActive={(tab: any) => { setActiveTab(tab); setIsMobileMenuOpen(false); }} isMinimized={isSidebarMinimized} />
          )}
          {canAccessQC && (
